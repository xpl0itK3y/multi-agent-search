...
- ✅ **Экспорт PDF/DOCX (2.3):** `GET /v1/research/{id}/export?format=pdf|docx` (переиспользует `report_export.generate_pdf/docx`, корректный `Content-Disposition` с UTF-8-именем из title/prompt, CORS expose) + кнопки PDF/DOCX в артефакт-панели (blob-скачивание). ⬜ Остаток F2: мобильная адаптация.
