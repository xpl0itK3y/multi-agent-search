...
- ✅ **Лесенка источников 30 / 60 / 120 (EASY / MEDIUM / HARD):** глубину подняли согласованно по трём слоям, иначе верхние источники не наберутся. **Search-supply** (`search_depth_profiles`): EASY 3×12, MEDIUM 4×16, HARD 6×24 (kept), candidate-urls/results подняты — supply ≥ analyzer-капа с запасом на дедуп. **Analyzer-пул** (`DEPTH_ANALYSIS_PROFILES`): `max_sources` 12/24/36→**30/60/120** (+ `per_task` 4/6/12→12/16/24, `per_domain` 2/3/4→3/4/6, `conflict/evidence_limit`→12/18/30). **Бюджет контента** `payload_char_budget` 12k/28k/42k→**30k/70k/140k** пропорционально (бюджет — общий, делится по источникам; без подъёма лишние голодали бы). **Writer**: MEDIUM кап секций 2→3 (~20/секц.), HARD кап 6 (~20/секц.); EASY одностадийный. Инвариант-тест лесенки (supply ≥ кап, плотность ≥ 900 симв/источник); профильные тесты переписаны на `_prepare_aggregated_data` (устойчиво к writer-пути). 286 тестов, eval-гейт чист. ⚠️ **Стоимость/латентность выросли** (MEDIUM ~4 LLM-вызова, HARD ~7; HARD фетчит до ~288 страниц через DDG → риск rate-limit). Путь к «1000» без банов и взрыва контекста — map-reduce-заметки + платный поиск (P2).
