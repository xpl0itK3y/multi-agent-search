...
- ✅ **Экспорт PDF/DOCX (2.3):** `GET /v1/research/{id}/export?format=pdf|docx` (переиспользует `report_export.generate_pdf/docx`, корректный `Content-Disposition` с UTF-8-именем из title/prompt, CORS expose) + кнопки PDF/DOCX в артефакт-панели (blob-скачивание). ✅ Мобильная адаптация: сайдбар = off-canvas drawer (backdrop, кнопка-меню, закрытие при навигации), `lg:` static; thread/артефакт стекаются в колонку (thread ≤45vh).
- ✅ **Streamlit декомиссирован:** `streamlit_app.py` удалён, зависимость убрана; в compose `ui`→`web` (Vue SPA через nginx, прокси `/v1`,`/health`,`/metrics` с `proxy_buffering off` для SSE); `report_export`/`report_utils` оставлены (их использует экспорт-API). README обновлён.
- ✅ **Wave 0 закрыт полностью:** `user_id` миграция (0.6) сделана вместе с auth (см. ниже); **eval-харнесс (0.5)** — `eval/` пакет: нормализованный `EvalSample`, 12 чистых метрик (уникальные источники/домены, доля high/medium, цитаты/плотность цитирования, покрытие плана, `must_mention_rate`, стоимость/токены/латентность), золотой набор `eval/datasets/gold.jsonl` (6 запросов), детерминированные фикстуры, `python -m eval` с таблицей/деления и **гейтом регрессий** против `eval/baseline.json` (5% допуск), live-раннер поверх `ResearchService`. 11 тестов (`tests/test_eval.py`), документация `docs/eval-harness.md`. Метрики эвристические → относительный сигнал vs бейзлайн/Gemini, не истина в последней инстанции.
...
- ✅ **Полировка UX (часть):** кликабельные `[Sn]` (ведут на URL источника, маппинг из раздела «Источники» отчёта); показ стоимости/токенов (`llm_token_usage` из `/status`); **светлая/тёмная тема** (CSS-переменные RGB-каналами → работают alpha-модификаторы; переключатель в сайдбаре, persist в localStorage; `prose dark:prose-invert`). ✅ **Действия в истории:** переименование (`PATCH /v1/research/{id}`, title в `graph_state`, отдаётся в `ResearchHistoryItem`), удаление (с подтверждением, навигация домой если открыт), клиентский поиск по истории (лупа → инлайн-поле, фильтр по title/prompt); скелетон загрузки; директива `v-focus` для инлайн-rename.
...
- ✅ **Sign in with Google (OAuth2):** Authorization-Code flow на `httpx` (без новых зависимостей) — `GET /v1/auth/google/login` (state-cookie CSRF → редирект на Google), `GET /v1/auth/google/callback` (проверка state → обмен `code` на токен нашим `client_secret` → Google userinfo → проверка `email_verified` → `get_or_create_oauth_user` (passwordless) → наш JWT в cookie → редирект на SPA). `GET /v1/auth/config` отдаёт `{google_oauth}` — фронт показывает кнопку «Войти через Google» условно. Покрыто тестами (полный flow с моком userinfo, bad-state→400, 404 без конфига, идемпотентность юзера). Нужны `GOOGLE_CLIENT_ID/SECRET` из Google Cloud + `AUTH_DISABLED=false`.
- ✅ **JWT-сессии (HS256, stdlib):** токен переведён с самодельного HMAC на **стандартный JWT** — `header.payload.signature`, base64url, claims `sub/email/iat/exp`, декодируется на jwt.io (без новых зависимостей). Аутентификация принимается из `Authorization: Bearer <jwt>` **или** из httpOnly-cookie (`_extract_token`); login/register отдают `AuthSession {access_token, token_type:"bearer", user}` и ставят cookie. Фронт хранит токен (localStorage) и шлёт Bearer; cookie — фолбэк (в т.ч. для SSE). Покрыто тестами (структура JWT, claims, tamper, Bearer-аут без cookie). Проверено вживую: register отдаёт валидный JWT. ⚠️ Для включения логина — `AUTH_DISABLED=false` + сильный `AUTH_SECRET_KEY`.
...
- ✅ **Эскалация чата в мини-поиск:** эвристика покрытия (токены вопроса vs пул источников); если не покрыто — `_mini_search_for_chat` запускает лёгкий `SearchAgent` (4 источника), персистит как задачу (попадает в `/sources`), ответ строится по обогащённому пулу. SSE-событие `searching` → индикатор «Ищу источники…» на фронте. Покрыто тестом.

- ✅ **Тред-чат: несколько дип-ресёрчей в одном разговоре.** Ресёрчи группируются в тред через `thread_id` (в `graph_state`, без миграции); `start_research` принимает/генерит его и возвращает; `GET /v1/threads/{id}` отдаёт ресёрчи треда по порядку; `ResearchHistoryItem.thread_id` → сайдбар группирует «Недавние» по тредам. Фронт: `ThreadView` (лента ходов + композер снизу — «не понравилось → запусти заново/иначе» создаёт новый ресёрч в том же треде) и переиспользуемый `ResearchTurn` (статус/SSE/план/уточнения → отчёт через `ArtifactPanel`). Покрыто сервис-тестом группировки. **Тред-композер имеет тумблер «Исследование / Быстрый вопрос»:** быстрый вопрос — grounded follow-up (стриминг + эскалация в мини-поиск) к последнему завершённому ресёрчу треда, рендерится Q&A-пузырями в той же ленте. _Q&A эфемерны в V1 (живут на ресёрче, не перезагружаются в тред при refresh)._
- ✅ **Защита от перегрузки (one-at-a-time):** нельзя стартовать новый дип-ресёрч, пока другой в работе. Бэкенд: `start_research` отдаёт **409**, если у пользователя есть ресёрч в `processing/analyzing` (лимит `max_concurrent_researches`, дефолт 1; в single-tenant — глобально). Фронт: тред-композер блокирует режим «Исследование» с подсказкой, пока любой ход треда не завершён (`@done`); «Быстрый вопрос» остаётся доступен. Покрыто тестом 409.
- ✅ **Аккуратная обработка ошибок финализации:** при сбое (например, 401 от LLM) вместо дампа сырой ошибки в отчёт сохраняется классифицированное сообщение (auth/rate-limit/timeout/generic) без утечки ключей.

- ✅ **P2 / 3.2 — кэш поиска (Postgres + TTL):** общий кэш результатов поиска (`search_cache` таблица + миграция `20260610_000015`), методы `get_cached_search`/`put_cached_search` на `TaskStore` (in-memory + SQLAlchemy). `SearchAgent` оборачивает поиск: ключ = `sha256(backend:max_results:query)`, при попадании в TTL (`SEARCH_CACHE_TTL_SECONDS`, дефолт 1 день) переиспользует результаты вместо обращения к (платному) Tavily. Шарится между воркерами. Запись кэша в try/except — не ломает поиск. Выключается `SEARCH_CACHE_ENABLED=false`. Покрыто 5 тестами (hit/miss/TTL, ключ, агент переиспользует/не кэширует при disabled). 303 теста, eval-гейт чист. ⚠️ Прогнать `alembic upgrade head` на Postgres при деплое.

- ✅ **P2 / 3.1 — Tavily search backend (снят потолок DDG):** абстракция `SearchBackend` в `providers/search.py` + `TavilyBackend` (primary) и `DuckDuckGoBackend` (фолбэк), селектор `build_search_backends` по `SEARCH_BACKEND` (auto/tavily/duckduckgo) и наличию `TAVILY_API_KEY`. `SearchProvider` пробует primary → при ошибке/пустоте падает на DDG. **Главное:** Tavily отдаёт `raw_content` (текст страницы) прямо в выдаче — `SearchAgent` использует его и **пропускает фетч/extract** для таких URL, что убирает хрупкий этап на ~сотни загрузок и DDG-rate-limit (узкое место глубоких тиров). Без ключа поведение прежнее (DDG). Новых зависимостей нет (httpx уже был). Конфиг в `.env.example`. Покрыто 12 тестами (маппинг полей, селектор, фолбэк, агент пропускает фетч). 298 тестов, eval-гейт чист. _3.3/3.4 (pgvector-заметки, map-reduce) не нужны — «1000» отложено; 3.2 (кэш поиска) — опционально._

- ✅ **Лесенка источников 15 / 60 / 120 (EASY / MEDIUM / HARD):** глубину подняли согласованно по трём слоям, иначе верхние источники не наберутся. **Search-supply** (`search_depth_profiles`): EASY 2×8, MEDIUM 4×16, HARD 6×24, candidate-urls/results подняты — supply ≥ analyzer-капа с запасом на дедуп. **Analyzer-пул** (`DEPTH_ANALYSIS_PROFILES`): `max_sources` 12/24/36→**15/60/120** (+ `per_task` 8/16/24, `per_domain` 2/4/6, `conflict/evidence_limit` 8/18/30). **Бюджет контента** `payload_char_budget`→**15k/70k/140k** пропорционально (бюджет — общий, делится по источникам; без подъёма лишние голодали бы). **Writer**: MEDIUM кап секций 3 (~20/секц.), HARD кап 6 (~20/секц.); **EASY оставлен быстрым одностадийным** (2 задачи, 1 LLM-вызов). **Тайм-бюджет** `finalize_budget_max_seconds` 240→**480** под тяжёлую финализацию HARD (EASY/MEDIUM финишируют раньше — потолок влияет только на HARD). Инвариант-тест лесенки (supply ≥ кап, плотность ≥ 900 симв/источник); профильные тесты переписаны на `_prepare_aggregated_data`. 286 тестов, eval-гейт чист. ⚠️ **MEDIUM/HARD подорожали** (≈4 / ≈7 LLM-вызовов; HARD фетчит до ~288 страниц через DDG → риск rate-limit). Путь к «1000» без банов — map-reduce-заметки + платный поиск (P2).

- ✅ **5.1 — многоступенчатый Writer на MEDIUM:** путь `outline → секции → сшивка` (`_run_parallel_section_analysis`) распространён с HARD на **MEDIUM** при ≥12 источниках — параллельные секции по чанкам + один синтез-проход даёт структурнее/полнее отчёт вместо одного большого вызова. MEDIUM ограничен: чанк 8, **≤2 секции** (2 секционных вызова + 1 синтез) — контроль стоимости/латентности. **EASY остаётся одностадийным** (быстрый/дешёвый), **HARD не тронут** (тесты подтверждают 30→3, 50→5 секций). Покрыто 9 тестами (`tests/test_writer_medium.py`). 281 тест зелёный, eval-гейт чист.

- ✅ **P3 — верификатор (5.2, 5.3):** `ReportCriticAgent` (детерминированный, без LLM): (1) **per-claim уверенность** из evidence-групп — `support_level` strong/medium/weak по числу подтверждающих источников; (2) **critic-проход «план vs отчёт»** — токен-оверлап описаний под-вопросов с телом отчёта (Sources вырезаются), `coverage_ratio` + список непокрытых; (3) **блок «не подтверждено»** — непокрытые под-вопросы + пометка о смягчённых утверждениях. Две markdown-секции инжектятся в отчёт между «Заключением» и «Источниками» (видны сразу, без фронта), плюс структурный `GET /v1/research/{id}/verification` (recompute по образцу `/conflicts`) и вкладка **Confidence** в артефакт-панели (бейджи уверенности, прогресс-бар покрытия, непокрытые вопросы; ru/en/es). Нулевая доп. стоимость LLM (eval-гейт зелёный). Покрыто 12 тестами (`tests/test_report_critic.py` + эндпоинт). **272 теста зелёные.**
