...
- ✅ **Глубокий HARD (deep tier, шаг 1):** HARD углублён ~2x — analyzer-пул `max_sources` 36→**72** (+ `per_task` 8→12, `per_domain` 4→5, `conflict/evidence_limit` 16→24), а `payload_char_budget` поднят 42000→**84000** пропорционально (бюджет — общий, делится по источникам; иначе лишние источники голодали бы). Search-сторона не тронута: HARD уже добывал ~120 кандидатов (6 задач × 20), анализатор был узким местом на 36 — теперь синтезирует из 72. Writer: чанк 12 без изменений + **кап 6 секций** (72/12) → ~6 секций того же размера + синтез (было 3+синтез); стоимость финализации ~2x, runtime по-прежнему под `finalize_budget_max_seconds=240`. EASY/MEDIUM не затронуты. Тесты подтверждают 72→6 секций; eval-гейт чист. _Путь к «1000»: следующий шаг — map-reduce-заметки + платный поиск (P2)._
