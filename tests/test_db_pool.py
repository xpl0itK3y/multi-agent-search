...
def test_engine_kwargs_sqlite_skips_pool_sizing():
    # SQLite ignores QueuePool sizing — only pre_ping (and hidden parameters) are set.
    assert _engine_kwargs("sqlite:///./dev.db") == {"pool_pre_ping": True, "hide_parameters": True}
