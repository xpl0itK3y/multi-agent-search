...
def test_research_summary_includes_graph_execution_summary():
    task_store = InMemoryTaskStore()
    research = task_store.add_research(
        ResearchRequest(prompt="linux vs macos", depth=SearchDepth.HARD),
        task_ids=["task-1", "replan-1", "replan-2"],
    )
    task_store.add_task(
        {
            "id": "task-1",
    ...
    task_store.add_task(
        {
            ...
            "queries": ["linux vs macos official source"],
            "status": TaskStatus.COMPLETED,
            "logs": ["Generated by ReplanAgent: the current source pool lacks strong primary-source coverage"],
        }
    )
    task_store.add_task(
        {
            ...
            "queries": ["linux vs macos stability official source"],
            "status": TaskStatus.COMPLETED,
            "logs": ["Generated by ReplanAgent: resolve conflicting evidence about stability"],
        }
    )

    summary = ResearchService(task_store=task_store).get_research_summary(research.id)

    assert summary.graph_execution_summary.branching_active is True
    assert summary.graph_execution_summary.follow_up_task_count == 2
    assert summary.graph_execution_summary.replan_task_count == 1
    assert summary.graph_execution_summary.tie_break_task_count == 1
